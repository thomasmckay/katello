module Katello
  class ContentViewDockerFilter < ContentViewFilter
    CONTENT_TYPE = DockerManifest::CONTENT_TYPE
    ...
    def query_manifests(repo, rule)
      ...
      query = DockerManifest.joins(:docker_tag).in_repositories(repo).where("#{DockerTag.table_name}.name ilike ?", query_name).uniq
      query.all.collect do |manifest|
        manifest.docker_tag.name
      end
    end
  end
end
