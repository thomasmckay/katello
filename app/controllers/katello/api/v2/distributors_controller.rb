  ...
  class Api::V2::DistributorsController < Api::V2::ApiController
    ...
    def_param_group :distributors do
      param :distributor, Hash, :required => true, :action_aware => true do
        ...
        param :installed_products, Array, :desc => "List of products installed on the distributor"
        param :service_level, String, :allow_nil => true, :desc => "A service level for auto-healing process, e.g. SELF-SUPPORT"
        param :release_version, String, :desc => "Release of the OS. The $releasever variable in repo url will be replaced with this value"
    ...
    param :subscription_id, String, :desc => "Filter distributors by subscribed pool"
...
=end
      ...
      filters << {:terms => {:uuid => Distributor.all_by_pool_uuid(params['subscription_id']) }} if params['subscription_id']
      ...
      respond_for_index(:collection => item_search(Distributor, params, options))
    end

    api :POST, "/environments/:environment_id/distributors", "Register a distributor in environment"
    api :POST, "/distributors", "Register a distributor in environment"
